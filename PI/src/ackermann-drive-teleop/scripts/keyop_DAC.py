...
class AckermannDriveStampedKeyop:
    ...
    def __init__(self, args):
        ...
        for key in key_bindings:
            key_bindings[key] = \
                    (key_bindings[key][0] * float(max_speed) / 63,
...
##############################
        ...
        if (self.speed <= -0.44 and self.speed >= -0.46) and (old_speed >= -0.01 and old_speed <= 0.01)  :
            rospy.sleep(0.5)
        if (self.speed <= 16 and self.speed >= 1)   :
            self.speed=16
        if (self.speed <= -1 and self.speed >= -16)   :
            self.speed=-16
################################
        rospy.loginfo('\x1b[1M\r'
        ...
        while 1:
            ...
            if key in key_bindings.keys():
                if key == control_keys['space']:
                    ...
                    if (current_time-last_time >=6.0):
		                self.steering_angle = \
		                        self.steering_angle + key_bindings[key][1]
		                self.steering_angle = clip(
		                    self.steering_angle,
		                    self.steering_angle_single_range[0],

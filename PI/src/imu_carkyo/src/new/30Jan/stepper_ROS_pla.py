...
#-ve is right is direction 1
    ...
    if enable == 1 and steps >= 0.05:
        ...
        with open("Stepper.csv", "a") as text_file:
            text_file.write("{0},-,{1},{2},-,{3},{4}\n".format(seq,steering_angle *180.0/math.pi,angle_val,steps,DIR))
        s = str(seq)+str(steering_angle *180.0/math.pi)+str(angle_val)+str(steps)+DIR
        rospy.loginfo(s)

...
#Very Low speed PID
        speedTempAcc = abs(speed)
        if mean_Vmps < speedTempAcc:
            if mode == "Reverse":
                speedTempAcc = speedTempAcc + 0.3
                if speedTempAcc > MIN_MOVING_SPEED+.1: #MAX_LOW_SPEED_DAC_VALUE:
                    speedTempAcc = MIN_MOVING_SPEED+.1 #MAX_LOW_SPEED_DAC_VALUE
            else:
                speedTempAcc = speedTempAcc + 0.2
                if speedTempAcc > MIN_MOVING_SPEED: #MAX_LOW_SPEED_DAC_VALUE:
                    speedTempAcc = MIN_MOVING_SPEED #MAX_LOW_SPEED_DAC_VALUE
            
        elif (mean_Vmps - speedTempAcc) > 0.1:
            speedTempAcc = speedTempAcc - 0.03

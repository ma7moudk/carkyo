...
def callback_move_base(data):
    global timeout_command_flag ,speed , Vmps_10  , pitch ,kp , mean_Vmps
    timeout_command_flag =0
    global old_speed ,mode_time,speedTemp,speedTempAcc,speed_counter
    speed=data.drive.speed
    print speed_counter ,Vmps_10
    last_speed_counter=speed_counter -1
    if(last_speed_counter < 0 ):
        last_speed_counter=9
    if ((speed != old_speed) or (Vmps_10[speed_counter] != Vmps_10[last_speed_counter]  ) ):
...
#            print "+ve speed"
 ...
 # waaaaaAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAarnig you should differ forward and reverse 
    mean_Vmps=np.mean(Vmps_10)             
    if ( ( (abs(mean_Vmps)-abs(speed) ) > 0.15 ) and (abs(mean_Vmps)>0.5 )  and (mean_Vmps != 0.0) ):
        print ("descending slope")
        error= abs(mean_Vmps) - abs(speed)
        modified=abs(speed) - kp * error
        speedTempAcc=modified
                
    elif ( ( (abs(speed) - abs(mean_Vmps) )> 0.15) and (abs(mean_Vmps)<1.5) and (mean_Vmps != 0.0) ): #ascending
        print("ascending slope")
        error= abs(speed) - abs(mean_Vmps)
  #      print ("error",error,kp * error) 
        modified=abs(speed) + kp * error   ##don't use speedTempAcc redo fixing every time
        speedTempAcc=modified
    ...
    if(time.time()-mode_time>1.0):
	    ...
	    if(int(DACvalue)<=50):
	        print "DAC",DACvalue,"mv_bs",speed,"modified",speedTempAcc,"sensor", mean_Vmps,"pitch",pitch*180.0/math.pi,rospy.Time.now()	    
	        accelerate(int(DACvalue))
	    else:
	        print "DAC>50,mv_bs,modified,sensor,pitch,now", DACvalue ,speed ,speedTempAcc, mean_Vmps , pitch*180.0/math.pi ,rospy.Time.now()

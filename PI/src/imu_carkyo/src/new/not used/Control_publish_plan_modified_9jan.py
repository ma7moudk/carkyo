...
def callback_key(data):
    global timeout_command_flag_key ,speed_key , Vmps_3_key  ,kp_key , mean_Vmps_key , command_angle_key ,angle_val
    timeout_command_flag_key =0
    global old_speed_key ,mode_time_key,speedTemp_key,speedTempAcc_key,speed_counter_key,vms_counter_key

    print "ERROR: allback_key(data):"
#    print ("vms_counter : ",vms_counter)
    vms_counter_key = 0
    command_angle_key=data.drive.steering_angle
    speed_key=data.drive.speed
    original_speed_key=speed_key
    if speed_key > 3.0:
        speed_key = 3.0
    elif speed_key < -3.0:
        speed_key = -3.0
    needed_angle_change_key=command_angle_key -(angle_val*math.pi/180.0)
#    print needed_angle_change , "needed_angle_change" , command_angle , angle_val
...
#        speedTempAcc=0.0
    ...
    if(time.time()-mode_time>1.0):
        DACvalue=(-4.4 * (DACSpeed * DACSpeed) ) + (26.4 * DACSpeed) +25
#        DACvalue = DACSpeed *63.0/5.0
...
def callback_move_base(data):
    global timeout_command_flag ,speed , Vmps_3 , pitch ,kp , mean_Vmps , command_angle ,angle_val
    print "callback_move_base: Last Speed", timeout_command_flag, speed
    timeout_command_flag =0
    global old_speed ,mode_time,speedTemp,speedTempAcc,speed_counter,vms_counter
...
#    print ("vms_counter : ",vms_counter)
    ...
    print "callback_move_base: New Speed", timeout_command_flag, speed
...
#    print needed_angle_change , "needed_angle_change" , command_angle , angle_val
    ...
    print "callback_move_base: 1", speed, old_speed
...
#            print "DAC",DACvalue,"mv_bs",speed,"modified",speedTempAcc,"sensor", mean_Vmps,"orginal", original_speed,rospy.Time.now()    
    ...
    old_speed=speed
    print "callback_move_base: End", timeout_command_flag, speed
#####$$$$$##### keyboard callback #####$$$$$##### 

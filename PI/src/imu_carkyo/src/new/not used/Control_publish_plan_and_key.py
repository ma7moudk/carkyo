    ...
    if (speed != old_speed):
        if (speed >= -0.05) and (speed <= 0.05)  : ### Case 0
            ...
            if (mode != "Reverse"):
                ...
                mode_time=time.time()
            speedTemp=speed
            print speedTemp , "before rev"
            speedTempAcc=translate(speedTemp, -5.00, -0.05, 63, 22)
            print speedTempAcc , "after rev"
            
        elif speed >= 0.05 : ##### Case 4
            if (mode !="Forward"):
                rospy.loginfo("mode wasn't forward ")
                change_mode("Forward")
                mode_time=time.time()
            speedTemp=speed
            print speedTemp , "before"
            speedTempAcc=translate(speedTemp, 0.05, 5.00, 22, 63) 
            print speedTempAcc , "after"
        else : ##### Case 5
            print "else"     

        if(time.time()-mode_time>1.0):
			print speedTempAcc , "error"

...
def timeout_fun(event):
    global timeout_command_flag , timeout_command_flag_key , jerk
    if (timeout_command_flag == 0 ):
        timeout_command_flag=1
        print "callback on time"
    else:
...
def callback_move_base(data):
    global timeout_command_flag
    timeout_command_flag =0
    global old_speed ,mode_time,speedTemp,speedTempAcc, speed
    speed=data.drive.speed
    #speed=translate(speed, -1.50, 1.50, -5.0, 5.0)
    if (speed != old_speed):
        if (speed >= -14) and (speed <= 14)  : ### Case 0
            speedTempAcc=13
            change_mode("Normal")

        elif speed <= -14 : ##### Case 3
            if (mode != "Reverse"):
                rospy.loginfo("mode wasn't backward ")
                change_mode("Reverse")
                mode_time=time.time()
            speedTempAcc=-speed
            print speedTempAcc , "range 22 ,63"
            
        elif speed >= 14 : ##### Case 4
            if (mode !="Forward"):
        ...
        if(time.time()-mode_time>1.0):
			print speedTempAcc , "acceleating"
...
def callback_speed(data): #add odommsg as global
    global mode ,lengthBetweenTwoWheels ,wheelBase ,angle_val, speedTempAcc, speed
...
def listener():
    rospy.Subscriber("/rbcar_robot_control/command1", AckermannDriveStamped, callback_move_base,queue_size=1)

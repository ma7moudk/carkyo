...
HUNDRED_MICRO = 0.0001 # milli temp = 1 sec (10.0 / 1000.0) / (1000.0)        # units in sec0
HunderedMicros = 0
Millies = 0
Secs = 0
Minutes = 0
...
def ResetStepperPos(): # ros service to set reset
	global stepper_pos
	print "reset"
	stepper_pos=0.0
def HundredMilliesTask()"
    #ROS publish stepper_pos
    pass

def GetStepperPos(): # ROS Message to be published periodically every 100 millies
	global stepper_pos
	return stepper_pos

def stepper_change(i): # ROS to be called from listner callback
    global stepper_pos

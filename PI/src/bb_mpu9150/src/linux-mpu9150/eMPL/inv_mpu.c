...
printf("-10- st.chip_cfg.sensors = %d", st.chip_cfg.sensors);
#if defined MPU6050
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    if (mpu_read_mem(MPU6500_MEM_REV_ADDR, 1, &rev))
       {printf("error3");  return -1; }
    if (rev == 0x1 || rev == 0x3 || rev == 0x6) // KZOH added rev == 3 , rev == 6
        st.chip_cfg.accel_half = 0;
    else {
        printf("\nWe are at mpu6500 mode  fhdhfdf\n");
        log_e("Unsupported software product rev %d.\n", rev);
        return -1;
        printf("erro4 unsupported");
    }
    ...
    if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
      {printf("error 5 i2c write ");  return -1;}
#endif
    ...
    if (mpu_set_gyro_fsr(2000))
      {printf("error6 set gyro");  return -1;}
    if (mpu_set_accel_fsr(2))
      {printf("error7 set acc");  return -1; }
    if (mpu_set_lpf(42))
      {printf("error 8 set lpf");  return -1;}
    if (mpu_set_sample_rate(50))
       {printf("error10 smpl rate");  return -1;}
...
{
    unsigned char tmp[2];
 //   printf("\nst.hw->addr = %4X st.reg->bank_sel = %4X st.reg->mem_r_w = %4X st.hw->bank_size =%d\n", st.hw->addr, st.reg->bank_sel, st.reg->mem_r_w, st.hw->bank_size);
    if (!data)
       {printf("read mem error not data"); return -1;}
  //  if (!st.chip_cfg.sensors)
    if (!(1))
       {printf("ream mem error not st chip config"); return -1;}
    ...
    /* Check bank boundaries. */
    if (tmp[1] + length > st.hw->bank_size)
       { printf("readmem error hiher bank size"); return -1;}

    if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
        {printf("read mem error i2c write " ); return -1;}
    if (i2c_read(st.hw->addr, st.reg->mem_r_w, length, data))
       { printf("readmem error i2c read");  return -1; }
    return 0;
}
